...
func (r *summaryRepository) GetByID(ctx context.Context, id string) (*model.Summary, error) {
	query := `SELECT id, user_id, source_type, file_url, content, summary, created_at FROM summaries WHERE id = $1`
	s := &model.Summary{}
	err := r.db.QueryRow(ctx, query, id).Scan(&s.ID, &s.UserID, &s.SourceType, &s.FileURL, &s.Content, &s.Summary, &s.CreatedAt)
	if err != nil {
		return nil, err
	}
	return s, nil
